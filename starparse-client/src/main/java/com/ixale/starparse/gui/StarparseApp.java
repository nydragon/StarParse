...
public class StarparseApp extends Application {
    ...
    public static String getConfigDir() {
        ...
        if (configDir == null) {
            ...
            configDir = homeDir.concat("/.config/StarParse/");
        }

        logger.info("Using following config directory: {}", configDir);
        return configDir;
    }

    public static String CONFIG_DIR = getConfigDir();

    public static String VERSION = com.ixale.starparse.domain.Application.VERSION,
            TITLE = "Ixale's StarParse",
            CONFIG_FILE = CONFIG_DIR.concat("starparse.xml"),
            CONFIG_TIMERS_FILE = CONFIG_DIR.concat("starparse-timers.xml"),
            CONFIG_ATTACKS_FILE = CONFIG_DIR.concat("starparse-attacks.xml"),
            SOUNDS_DIR = "sounds",
            ICONS_DIR = "icons";
    ...
    public void start(final Stage stage) throws Exception {
        ...
        StatusBarAppender.setListener(new StatusBarAppender.Listener() {
            ...
            public void onMessage(final String msg, final LoggingEvent event) {
                ...
                if (event.getLevel().equals(Level.ERROR)) {
                    if (lastErrorReported != null
                            && (lastErrorReported + ERROR_REPORT_INTERVAL > System.currentTimeMillis())) {
                        // too soon
                        return;
                    }
        ...
        stage.getIcons().add(new Image(Objects.requireNonNull(
                getClass().getClassLoader().getResourceAsStream("img/Star-Wars-The-Old-Republic-8-icon.png"))));
        stage.show();
    ...
    private void addResizingNode(final Node node, final int minW, final int minH) {
        ...
        node.setOnMousePressed(me -> {
            ...
            initialX = (node.getScene().getWindow().getX() + node.getScene().getWindow().getWidth()) - me.getScreenX()
                    - 20;
            initialY = (node.getScene().getWindow().getY() + node.getScene().getWindow().getHeight()) - me.getScreenY()
                    - 20;
            me.consume();
        });
        ...
        node.setOnMouseDragged(me -> {
            ...
            node.getScene().getWindow()
                    .setWidth(Math.max(minW, me.getScreenX() + initialX - node.getScene().getWindow().getX() + 20));
            node.getScene().getWindow()
                    .setHeight(Math.max(minH, me.getScreenY() - initialY - node.getScene().getWindow().getY() + 20));
    ...
    public static void setStylesheets(final Stage stage, final MainPresenter mainPresenter, final boolean isDark) {
        ...
        if (javafx.stage.Screen.getPrimary().getDpi() > 86) { // 125% = 96 on high-DPI screens // for whatever reason my
                                                              // screen is at 87 and I need this
                ...
                new TimerTask() {
                    ...
                    public void run() {
                        ...
                        if (offset == null) {
                            ...
                            // "Unable to contact time server to resolve local clock difference, timers may
                            // be off",
                            // Type.ERROR);
                            // }
                            // });
